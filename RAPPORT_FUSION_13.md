# Rapport de fusion #13 — thrive-main-fusionne (fusion #12) + Cresyva Telegram Omnichannel v3 + patch affiliation
...
1. **`thrive-main-fusionne.zip`** (511 fichiers) — état documenté par
   `RAPPORT_FUSION_12.md` : base CRESYVA + connexion Google OAuth + lien
...
2. **`Cresyva-CRESYVA-telegram-omnichannel-v3.zip`** (523 fichiers) —
   même base CRESYVA, divergée séparément avec la fonctionnalité
...
ici pour l'historique), 13 propres à cresyva (tout le lot Telegram
Omnichannel), 51 fichiers présents des deux côtés mais divergents.

Cresyva a été pris comme **base** (c'est la branche la plus avancée : elle
...
apport de fusion #12 à reporter tel quel (aucun changement cresyva sur ce
fichier), évolution cresyva à conserver telle quelle (rien à reporter),
...
## 1. Apports de la fusion #12 reportés sur la base cresyva
...
conserve en plus les classes propres à cresyva (`.site-editor-*`,
...
**conservé** — la branche cresyva développe activement l'affiliation
...
Le passage en `CRESYVA` de ce fichier côté cresyva a aussi "compacté" le
...
fonctions de la version cresyva appelaient toutes leur `redirect()`/
...
gardant le texte "CRESYVA" et les couleurs de la version cresyva.

## 3. Vrai bug trouvé et corrigé — `affiliate-admin-service.ts`

`npm run typecheck` sur le résultat fusionné a révélé une erreur
préexistante dans le zip cresyva lui-même (indépendante de cette
...
cresyva) : le tableau de liens de nav `[["/affiliate/dashboard",
