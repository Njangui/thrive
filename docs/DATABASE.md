...
| `0058_whatsapp_coexistence_dedicated_numbers.sql` | WhatsApp Coexistence + numéro dédié aux Groupes : `provider_connections.provider_type` accepte `whatsapp_groups` (second profil Zernio par organisation, distinct de la messagerie en Coexistence) ; `whatsapp_groups.status` accepte `suspended` (numéro dédié repris faute de paiement — exclu du quota : `entitlements-service` ne compte que `connected`) ; `phone_numbers.current_period_end` / `last_renewal_reminder_sent_at` (loyer mensuel, cycle indépendant du forfait) + policy de lecture pour les membres de l'organisation ; nouvelle table `phone_number_requests` (une demande `pending` au plus par organisation — index unique partiel) ; `subscription_payments.payment_type` accepte `dedicated_number` + colonne `phone_number_id`. **Renumérotée de `0055` à `0058` à la fusion #15** (`0055` = Telegram, `0056`/`0057` = Catalogue V2) ; contenu SQL inchangé. ⚠️ Cette migration REMPLACE trois contraintes CHECK (`drop constraint` puis `add constraint`) : à appliquer une seule fois |
| `0059_catalog_videos.sql` | `catalog_videos` : vidéos du catalogue (produits/prestations) hébergées chez Zernio, **7 jours** (`expires_at`), index par organisation/échéance/produit/prestation/URL, RLS membre d'organisation. Voir `docs/CATALOG_VIDEOS.md`. **Renumérotée de `0058` à `0059` à la fusion #16** (`0058` = WhatsApp Coexistence) |
| `0060_landing_analytics.sql` | `analytics_events` : la contrainte CHECK des types d'événement est recréée pour accepter `video_play` (liste = `ANALYTICS_EVENT_TYPES` de `analytics-service.ts`, **à garder synchronisées** : `trackEvent` ne lève jamais, un type refusé par la base se perdrait silencieusement) + index `(organization_id, created_at desc)`. Voir `docs/LANDING_ANALYTICS.md`. **Renumérotée de `0059` à `0060` à la fusion #16** |
| `0061_freemium_plan.sql` | Passage en freemium : `plans.key` accepte `free` (affiché « Discover », 0 FCFA, permanent) ; Starter 15 000 FCFA, Pro 30 000 FCFA ; limites `plan_entitlements` des trois paliers ; **les organisations en essai (`trialing`) basculent en `free`/`active`** et celles sur l'ex-palier `business` retombent sur `starter`. ⚠️ Migration de DONNÉES : relire avant de l'appliquer sur une base réelle. **Renumérotée de `0056` à `0061` à la fusion #16** |

> Note (fusions #14 à #16) : ce tableau s'arrête à `0042` alors que les
> migrations `0043` à `0055` existent dans le dépôt — retard
> préexistant, non traité par ces fusions (seules les migrations
> qu'elles ajoutent, `0056` à `0061`, sont documentées ci-dessus). Le
