...
const domain = getDomainProvider(); // -> OpenProviderAdapter si configuré, sinon ManualDomainAdapter
const email = await getEmailProvider(); // -> ResendAdapter si configuré, sinon ConsoleLogEmailAdapter (jamais un crash)
```
...
construire. Pour la messagerie/le social/l'IA, la résolution du
credential est PAR TENANT (`infrastructure/providers/secrets-resolver.ts
::resolveCredential`) : un tenant avec son propre compte Zernio/IA
configuré utilise SON credential (Supabase Vault, jamais en clair), en
son absence le registry retombe sur la clé plateforme partagée — voir
`docs/SECURITY.md`.
