...
## U. Clôture — état du projet à l'issue de la vague B→S (master prompt)

Section ajoutée à la fusion des Lots 1/2/4 (RAPPORT_FUSION_6.md) —
périmètre hérité du Lot O, jamais réalisé lors de la vague K-O
précédente. Les sections A-T ci-dessus datent de la toute première
vague (B-E) et documentent des décisions de conception, pas l'état
actuel — elles restent en place comme historique, pas comme référence
de statut courant (voir `docs/ROADMAP.md`/`docs/MVP_SCOPE.md` pour ça).

**Vagues de développement traversées** :
- B-E (fondation multi-tenant, catalogue, orchestrateur, landing v1)
- F-J minus J (WhatsApp/social/IA/finance/analytics, Lot J jamais livré)
- K-O minus O (landing configurable par secteur, équipe/CRM/IA écrans,
  groupes WhatsApp activables, facturation récurrente/domaines/
  credentials par tenant ; Lot O jamais livré)
- Master prompt d'audit, découpé en Lots 1 (architecture/sécurité/DB/
  stock), 2 (dashboard/catalogue/landing), 3 (WhatsApp/IA/social — **pas
  encore livré**), 4 (Super Admin/abonnements/domaines)

**Ce qui a été réellement corrigé/complété par cette dernière vague**
(au-delà de ce que chaque `RAPPORT_LOT_*.md` individuel documente déjà) :
- une vraie race condition sur la transaction commande/stock (Lot 1) ;
...
- la landing marketing flexco  (remplace une page de statut de
...
- deux bugs réels trouvés à la fusion elle-même (typage + hoisting
  Vitest, Lot 1 — voir `RAPPORT_FUSION_6.md` section 3), confirmant que
  les lots livrés sans accès réseau à leur outillage restent un vrai
  facteur de risque, pas une formalité ;
- le test d'isolation multi-tenant réel, les tests de bordure
  entitlements paramétrés, le câblage `product_click`, une action
  "Réapprovisionner" pour le stock, la migration de `team-service.ts`
  vers `resolveRequestOrigin()`, et le script `seed:demo` — tous
  construits directement à la fusion, hors du périmètre qu'aucun des 4
  lots n'avait couvert (voir `RAPPORT_FUSION_6.md`).

**Ce qui reste ouvert** : voir `docs/ROADMAP.md`, section "Ce qui reste
réellement ouvert" — au moment d'écrire cette section, le point le plus
important est l'arrivée du Lot 3 et l'exécution réelle (jamais faite à
ce jour) du test d'isolation et du seed de démo contre une vraie
instance Supabase.
