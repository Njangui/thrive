...
/**
 * Tests de bordure paramétrés — périmètre hérité du Lot O (cahier
 * "Lot S/4", partie 6.2) : 99/100, 100/100, 101/100 sur CHAQUE
 * entitlement_key connue au moment de cette vague, plus -1 (illimité)
 * jamais confondu avec "0 restant". Volontairement séparés des tests
 * ci-dessus (qui couvrent déjà la même logique avec d'autres valeurs) :
 * ce bloc existe pour que la couverture demandée soit visible et
 * vérifiable telle quelle, pas seulement déductible d'autres tests.
 *
 * Les 4 clés n'empruntent pas toutes le même chemin dans
 * `canUseFeature` (voir le fichier source) — chaque cas choisit donc le
 * mock pertinent pour SA clé plutôt qu'un seul harnais générique :
 * - ai_credits    → délégué à getCreditStatus() (used/limit propres)
 * - whatsapp_groups → cumulatif, used = countOrganizationRows(...)
 * - broadcast_contacts / social_accounts → "par action", used toujours 0,
 *   c'est requestedAmount qui varie contre la limite du plan
 */
describe("Tests de bordure — 99/100, 100/100, 101/100 par entitlement_key", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetOrganizationAddonBonus.mockResolvedValue(0);
    mockHasDedicatedPhoneNumber.mockResolvedValue(false);
  ...
  describe("ai_credits (délégué à getCreditStatus)", () => {
    const creditStatus = (usedCredits: number) => ({
      organizationId: "org-1",
      includedCredits: 100,
      usedCredits,
      remainingCredits: Math.max(100 - usedCredits, 0),
    });

    it("99/100 utilisés : la 100e génération est autorisée", async () => {
      mockGetCreditStatus.mockResolvedValue(creditStatus(99));
      expect(await canUseFeature("org-1", "ai_credits", 1)).toEqual({
        allowed: true,
        limit: 100,
        used: 99,
        remaining: 1,
    ...
    it("100/100 utilisés : une génération de plus est refusée", async () => {
      mockGetCreditStatus.mockResolvedValue(creditStatus(100));
      expect(await canUseFeature("org-1", "ai_credits", 1)).toEqual({
        allowed: false,
        limit: 100,
        used: 100,
    ...
    it("101/100 (déjà au-delà — état de bordure improbable mais jamais un crash) : reste refusé, remaining ne descend jamais sous 0", async () => {
      mockGetCreditStatus.mockResolvedValue(creditStatus(101));
      const result = await canUseFeature("org-1", "ai_credits", 1);
      expect(result.allowed).toBe(false);
      expect(result.remaining).toBe(0);
    });

    it("plan illimité (-1) : jamais confondu avec '0 restant', quel que soit l'usage déjà consommé", async () => {
      mockGetCreditStatus.mockResolvedValue({
        organizationId: "org-1",
        includedCredits: -1,
        usedCredits: 100_000,
  ...
  describe("whatsapp_groups (cumulatif, countOrganizationRows)", () => {
    ...
    it("99/100 groupes connectés : en créer un 100e est autorisé", async () => {
      mockCountOrganizationRows.mockResolvedValue(99);
      expect(await canUseFeature("org-1", "whatsapp_groups", 1)).toEqual({
        allowed: true,
        limit: 100,
        used: 99,
        remaining: 1,
    ...
    it("100/100 groupes connectés : un 101e est refusé", async () => {
      mockCountOrganizationRows.mockResolvedValue(100);
      expect(await canUseFeature("org-1", "whatsapp_groups", 1)).toEqual({
        allowed: false,
        limit: 100,
        used: 100,
    ...
    it("101/100 (déjà au-delà) : reste refusé, remaining ne descend jamais sous 0", async () => {
      mockCountOrganizationRows.mockResolvedValue(101);
      const result = await canUseFeature("org-1", "whatsapp_groups", 1);
      expect(result.allowed).toBe(false);
      expect(result.remaining).toBe(0);
    });

    it("plan illimité (-1) : jamais confondu avec '0 restant' — countOrganizationRows n'est même pas appelé", async () => {
      mockGetEntitlementLimit.mockResolvedValue(-1);
      const result = await canUseFeature("org-1", "whatsapp_groups", 1);
      expect(result).toEqual({ allowed: true, limit: -1, used: 0, remaining: -1 });
      expect(mockCountOrganizationRows).not.toHaveBeenCalled();
  ...
  describe.each([["broadcast_contacts"], ["social_accounts"]])("%s (\"par action\", used=0)", (key) => {
    ...
    it("demande de 99 sur 100 : autorisée", async () => {
      expect(await canUseFeature("org-1", key, 99)).toEqual({ allowed: true, limit: 100, used: 0, remaining: 100 });
    });

    it("demande de 100 sur 100 : autorisée (pile à la limite)", async () => {
      expect(await canUseFeature("org-1", key, 100)).toEqual({ allowed: true, limit: 100, used: 0, remaining: 100 });
    });

    it("demande de 101 sur 100 : refusée", async () => {
      const result = await canUseFeature("org-1", key, 101);
      expect(result.allowed).toBe(false);
    });

    it("plan illimité (-1) : jamais confondu avec '0 restant', même pour une demande énorme", async () => {
      mockGetEntitlementLimit.mockResolvedValue(-1);
      expect(await canUseFeature("org-1", key, 100_000)).toEqual({ allowed: true, limit: -1, used: 0, remaining: -1 });
    });
  });

  it("le bonus add-ons s'additionne correctement à la limite du plan à chaque cas de bordure (whatsapp_groups, 99/100 -> 99/103)", async () => {
    // Plan à 100, +3 via add-ons (Lot G) : la vraie limite effective est
    ...
    // Sans le bonus, 99/100 + 4 aurait été refusé (99+4=103 > 100). Avec
    // le bonus (limite réelle 103), c'est pile autorisé.
    const result = await canUseFeature("org-1", "whatsapp_groups", 4);
    expect(result).toEqual({ allowed: true, limit: 103, used: 99, remaining: 4 });
  });
});
