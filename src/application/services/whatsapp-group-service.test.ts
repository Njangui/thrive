...
describe("formatGroupBroadcastMessage", () => {
  ...
  it("liste chaque produit avec nom, prix, description, lien — jamais 'undefined'", () => {
    const message = formatGroupBroadcastMessage(PRODUCTS, "https://monsalon.tokoo .app");
    ...
    expect(message).toContain("https://monsalon.tokoo .app/produits/sac-en-cuir");
    expect(message).not.toContain("undefined");
  });

  it("retourne une chaîne vide pour un catalogue vide (jamais un message vide envoyé aux groupes)", () => {
    expect(formatGroupBroadcastMessage([], "https://monsalon.tokoo .app")).toBe("");
  });
});
