 ...
 * pas seulement pour les posts publiés via le composer CRESYVA (seul cas
 ...
 * via CRESYVA (déjà existant, jamais recréé/modifié ici) ou détecté
  ...
  if (!socialPostId) {
    ...
    const { data: created, error: insertPostError } = await supabase
      ...
      .insert({
        ...
        content: "Publication détectée automatiquement (publiée hors CRESYVA).",
        status: "published",
