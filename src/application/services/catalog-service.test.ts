...
function makeBuilder(table: string) {
  ...
  const builder: Record<string, unknown> = {
    ...
    eq: vi.fn(() => builder),
    order: vi.fn(() => builder),
    // Passthrough générique — la vitrine publique (`listActiveProductsForStorefront`/
    // `listPromotedProductsForStorefront`, landing-config-service.ts) chaîne
    // ces filtres avant de résoudre via `then` ; aucun test ci-dessous n'a
    // besoin de vérifier LEUR filtrage exact (déjà couvert par leur
    // propre lecture de code), seulement le mapping image/prix qui suit.
    range: vi.fn(() => builder),
    not: vi.fn(() => builder),
    ilike: vi.fn(() => builder),
    in: vi.fn(() => builder),
...
describe("listActiveProductsForStorefront — photo principale (amélioration post-vague)", () => {
  it("choisit la photo de plus petite `position` comme image principale, quel que soit l'ordre renvoyé par la DB", async () => {
      ...
      data: [
        {
          ...
          description: null,
          categories: null,
          // Volontairement dans le désordre : la position fait foi, pas
          // l'ordre du tableau renvoyé par PostgREST.
          product_images: [
            { url: "https://x/secondaire.jpg", position: 1 },
            { url: "https://x/principale.jpg", position: 0 },
          ],
        },
      ],
      error: null,
    ...
    const [product] = await listActiveProductsForStorefront("org-1");
    expect(product!.imageUrl).toBe("https://x/principale.jpg");
  });

  it("renvoie imageUrl: null pour un produit sans aucune photo (jamais une erreur)", async () => {
    tableResults.set("products", {
      data: [{ id: "p1", name: "Sac", slug: "sac", unit_price: 10000, description: null, categories: null, product_images: [] }],
      error: null,
    });
