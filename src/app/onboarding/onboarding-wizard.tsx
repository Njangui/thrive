...
export function OnboardingWizard({ countries = [], initialStep = 1, initialOrganizationId = null }: { countries?: PublicCountry[]; initialStep?: number; initialOrganizationId?: string | null }) {
  ...
  return <main className="onboarding-page"><div className="onb-topbar"><flexco Brand compact dark /><span className="onb-save">Configuration sauvegardée automatiquement</span></div><div className="onb-layout"><aside className="onb-side"><span className="auth-kicker">Bienvenue</span><h2>Construisons votre espace.</h2><p>Quelques étapes suffisent pour adapter flexco  à votre activité. Vous pourrez tout modifier plus tard.</p><div className="onb-side-list">{["Votre activité", "Votre identité", "Vos coordonnées", "Votre catalogue", "Vos réponses", "Prêt à démarrer"].map((x, i) => <div key={x} className={step === i + 1 ? "active" : step > i + 1 ? "done" : ""}><span>{step > i + 1 ? "✓" : i + 1}</span>{x}</div>)}</div></aside><section className="onb-card">{isResuming && !error && <div className="onb-resume">↻ Vous reprenez une configuration déjà commencée.</div>}{error && <p className="adm-alert-danger">{error}</p>}
