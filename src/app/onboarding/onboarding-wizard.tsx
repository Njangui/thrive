...
export function OnboardingWizard({ countries = [], initialStep = 1, initialOrganizationId = null }: { countries?: PublicCountry[]; initialStep?: number; initialOrganizationId?: string | null }) {
  ...
  function businessSubmit(formData: FormData) { setError(null); startTransition(async () => { const result = await submitBusinessStep(String(formData.get("name") ?? ""), String(formData.get("industry") ?? industry), String(formData.get("countryCode") ?? selectedCountryCode)); if (!result.ok || !result.organizationId) return setError(result.error ?? "Impossible de créer votre entreprise."); setOrganizationId(result.organizationId); setStep(2); }); }
  ...
  return <main className="onboarding-page"><div className="onb-topbar"><span className="auth-logo-mark">S</span><span>SME-OS</span><span className="onb-save">Configuration sauvegardée automatiquement</span></div><div className="onb-layout"><aside className="onb-side"><span className="auth-kicker">Bienvenue</span><h2>Construisons votre espace.</h2><p>Quelques étapes suffisent pour adapter SME-OS à votre activité. Vous pourrez tout modifier plus tard.</p><div className="onb-side-list">{["Votre activité", "Votre identité", "Vos coordonnées", "Votre catalogue", "Vos réponses", "Prêt à démarrer"].map((x, i) => <div key={x} className={step === i + 1 ? "active" : step > i + 1 ? "done" : ""}><span>{step > i + 1 ? "✓" : i + 1}</span>{x}</div>)}</div></aside><section className="onb-card">{isResuming && !error && <div className="onb-resume">↻ Vous reprenez une configuration déjà commencée.</div>}{error && <p className="adm-alert-danger">{error}</p>}

{step === 1 && <><StepHeader step={1} title="Parlons de votre entreprise" description="Choisissez votre secteur : SME-OS adaptera automatiquement votre tableau de bord et vos outils."/><form onSubmit={onForm(businessSubmit)} className="onb-form"><Field label="Nom de l’entreprise"><input name="name" required autoFocus className="adm-input" placeholder="Ex. Kamenur Shop" /></Field><Field label="Pays"><select name="countryCode" value={selectedCountryCode} onChange={e => setSelectedCountryCode(e.target.value)} required className="adm-input">{countries.length ? countries.map(c => <option key={c.isoCode} value={c.isoCode}>{c.name} · {c.currencyCode}</option>) : <option value="">Aucun pays disponible</option>}</select></Field><div className="onb-label-row"><span>Votre activité</span><small>Cette sélection pilote votre interface</small></div><div className="industry-grid">{INDUSTRY_OPTIONS.map(opt => <button type="button" key={opt.value} onClick={() => setIndustry(opt.value)} className={`industry-option ${industry === opt.value ? "selected" : ""}`}><span className="industry-icon">{opt.icon}</span><span><b>{opt.label}</b><small>{opt.desc}</small></span>{industry === opt.value && <i>✓</i>}</button>)}</div><input type="hidden" name="industry" value={industry} /><button disabled={isPending || !countries.length} className="adm-btn-primary onb-submit">{isPending ? "Création..." : "Continuer"}<span>→</span></button></form></>}
