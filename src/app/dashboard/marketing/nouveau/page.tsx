...
export default async function NewPublicationPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [products, targets] = await Promise.all([getActiveProducts(organizationId, 60), listOmnichannelPublicationTargets(organizationId)]);
  ...
  async function publishAction(formData: FormData) {
    ...
    try {
      ...
      redirect(`/dashboard/marketing?success=${status}&count=${result.published + result.scheduled}&failed=${result.failed.length}`);
    } catch (err) {
      const message = err instanceof AppError ? err.message : err instanceof Error ? err.message : "Impossible de créer la publication.";
      redirect(`/dashboard/marketing/nouveau?error=${encodeURIComponent(message)}`);
    }
  }
