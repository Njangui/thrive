...
async function connectSocialAction(formData: FormData) {
  ...
  let url: string;
  try {
    url = await getZernioConnectUrl(organizationId, organization?.name ?? "Entreprise Flexco", platform as never, reconnectAccountId ? { reconnectAccountId } : undefined);
  } catch (error) {
...
async function connectWhatsAppAction(formData: FormData) {
  ...
  let url: string;
  try {
    url = await getZernioWhatsAppConnectUrl(organizationId, organization?.name ?? "Entreprise Flexco");
  } catch (error) {
...
async function connectWhatsAppGroupsAction(formData: FormData) {
  ...
  let url: string;
  try {
    url = await getZernioWhatsAppGroupsConnectUrl(organizationId, organization?.name ?? "Entreprise Flexco");
  } catch (error) {
...
/** Chemin payant — le commerçant demande à Flexco de lui fournir un numéro dédié (traité ensuite depuis /admin/numbers). */
async function requestDedicatedNumberAction(formData: FormData) {
  ...
  flash("success", "Demande envoyée — l'équipe Flexco va vous assigner un numéro sous peu.");
}
    ...
    <div className="space-y-7">
      <header className="relative overflow-hidden rounded-3xl bg-navy-900 p-6 text-white shadow-[0_20px_60px_-35px_rgba(14,17,48,.75)] sm:p-8">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="adm-eyebrow text-violet-300">Canaux</p><h1 className="mt-2 font-jakarta text-2xl font-extrabold tracking-tight sm:text-3xl">Tout connecter, sans complexité.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/60">Connectez vos canaux depuis Flexco. Les détails techniques restent derrière l&apos;interface.</p></div>
        ...
        {whatsappGroupsAccount ? (
          ...
          <div className="grid gap-0 sm:grid-cols-2">
            ...
            <div className="border-t border-navy-900/[0.06] p-5 sm:p-6">
              <p className="text-sm font-semibold">Je veux que Flexco m&apos;en fournisse un</p>
