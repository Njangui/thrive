...
export function LineAreaChart({
  ...
  const areaPath = `${linePath} L ${coords[coords.length - 1].x.toFixed(1)} ${(PADDING_TOP + plotHeight).toFixed(1)} L ${coords[0].x.toFixed(1)} ${(PADDING_TOP + plotHeight).toFixed(1)} Z`;
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-[220px] w-full" preserveAspectRatio="none">
      ...
      <title>{points.map((p) => `${p.label}: ${fmt(p.value)}`).join(" • ")}</title>
    </svg>
  );
}
