...
export default async function GroupsPage({
  ...
  // Lot M — file d'attente d'activation : groupes connectés côté CRESYVA
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-jakarta text-2xl font-bold tracking-tight">Groupes WhatsApp</h1>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-navy-900/[0.06] bg-white shadow-[0_1px_2px_rgba(16,23,49,0.04)] p-4">
        <div className="flex items-center justify-between">
          <h2 className="font-jakarta text-lg font-semibold">Groupes connectés</h2>
          <form action={syncGroupsAction}>
            <input type="hidden" name="organizationId" value={organizationId} />
            <SubmitButton
              pendingLabel="Synchronisation..."
              className="rounded-xl border border-navy-900/10 px-3 py-1.5 text-xs font-medium text-navy-900 hover:bg-navy-900/5 disabled:opacity-60"
            >
              Synchroniser
            </SubmitButton>
          </form>
        ...
        {connectedGroups.length === 0 ? (
          <p className="text-sm text-slate-500">Aucun groupe connecté pour l&apos;instant.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {connectedGroups.map((g) => (
                <tr key={g.id} className="border-b border-navy-900/5 last:border-0">
                  <td className="px-2 py-2">{g.name}</td>
                  <td className="px-2 py-2 text-slate-500">{g.participantCount ?? "—"}</td>
                  <td className="px-2 py-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs ${
                        g.status === "connected" ? "bg-success-50 text-success-700" : "bg-danger-50 text-danger-700"
                      }`}
                    >
                  ...
                  <td
                    className="px-2 py-2"
                    title="Une diffusion ne peut atteindre ce groupe que si CRESYVA dispose déjà d'une conversation active avec lui."
                  >
                    {g.isSendable ? (
        ...
        {pendingActivationGroups.length > 0 && (
          <div className="flex flex-col gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm">
            <div>
            ...
            <form action={syncGroupsAction}>
              ...
              <SubmitButton
                ...
                className="w-fit rounded-xl border border-amber-600/40 bg-white px-3 py-1.5 text-xs font-medium text-amber-700 hover:bg-amber-500/10 disabled:opacity-60"
              >
                Vérifier maintenant
              </SubmitButton>
            </form>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-navy-900/[0.06] bg-white shadow-[0_1px_2px_rgba(16,23,49,0.04)] p-4">
        <h2 className="font-jakarta text-lg font-semibold">Connecter un groupe</h2>
          ...
          <form action={connectGroupsAction} className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              {connectableAvailable.map((g) => (
                <label key={g.externalId} className="flex items-center gap-2 rounded-xl border border-navy-900/10 px-3 py-2 text-sm">
            ...
            <SubmitButton
              ...
              className="w-fit rounded-xl bg-violet-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
            >
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-navy-900/[0.06] bg-white shadow-[0_1px_2px_rgba(16,23,49,0.04)] p-4">
        <h2 className="font-jakarta text-lg font-semibold">Programmer une diffusion</h2>

        {sendableConnectedGroups.length === 0 ? (
          <p className="text-sm text-slate-500">Connectez au moins un groupe pour pouvoir programmer une diffusion.</p>
        ) : products.length === 0 ? (
          <p className="text-sm text-slate-500">
          ...
          <form action={createBroadcastAction} className="flex flex-col gap-4">
            ...
            <div>
              <p className="mb-2 text-xs font-medium uppercase text-slate-500">Produits à diffuser</p>
              <div className="flex flex-col gap-1">
                {products.map((p) => (
                  <label key={p.id} className="flex items-center gap-2 rounded-xl border border-navy-900/10 px-3 py-2 text-sm">
            ...
            <div>
              <p className="mb-2 text-xs font-medium uppercase text-slate-500">Groupes ciblés</p>
              <div className="flex flex-col gap-1">
                {sendableConnectedGroups.map((g) => (
                  <label key={g.id} className="flex items-center gap-2 rounded-xl border border-navy-900/10 px-3 py-2 text-sm">
                    <input type="checkbox" name="groupIds" value={g.id} className="h-4 w-4" />
                    {g.name}
                    {!g.isSendable && (
                      <span className="text-xs text-slate-500">(pas encore diffusable — voir colonne ci-dessus)</span>
                    )}
                  </label>
                ))}
              </div>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium uppercase text-slate-500" htmlFor="scheduledAt">
      ...
      {/* Historique */}
      <section className="flex flex-col gap-3 rounded-2xl border border-navy-900/[0.06] bg-white shadow-[0_1px_2px_rgba(16,23,49,0.04)] p-4">
        <h2 className="font-jakarta text-lg font-semibold">Historique des diffusions</h2>

        {broadcasts.length === 0 ? (
          <p className="text-sm text-slate-500">Aucune diffusion pour l&apos;instant.</p>
        ) : (
          <table className="w-full text-sm">
            <thead className="border-b border-navy-900/10 text-left text-xs uppercase text-slate-500">
              <tr>
            ...
            <tbody>
              {broadcasts.map((b) => (
                <tr key={b.id} className="border-b border-navy-900/5 last:border-0">
                  <td className="px-2 py-2 text-slate-500">{new Date(b.scheduledAt).toLocaleString("fr-FR")}</td>
                  <td className="px-2 py-2">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${BROADCAST_STATUS_STYLES[b.status] ?? "bg-slate-100 text-navy-900"}`}>
                  ...
                  <td className="px-2 py-2">
                    ...
                    {b.failedCount > 0 && <span className="text-danger-600"> · {b.failedCount} échec(s)</span>}
                  </td>
                  <td className="px-2 py-2 text-right">
                    <div className="flex items-center justify-end gap-3">
                      <Link href={`/dashboard/groups/broadcasts/${b.id}`} className="text-xs font-medium text-violet-600 hover:underline">
                        Détail
                      </Link>
                      {b.status === "scheduled" && (
                      ...
                      {(b.status === "completed" || b.status === "failed") && b.failedCount > 0 && (
                        <form action={retryBroadcastAction}>
                          <input type="hidden" name="organizationId" value={organizationId} />
                          <input type="hidden" name="broadcastId" value={b.id} />
                          <SubmitButton pendingLabel="..." className="text-xs font-medium text-navy-900 hover:underline disabled:opacity-60">
                            Relancer les échecs
                          </SubmitButton>
                        </form>
                      )}
