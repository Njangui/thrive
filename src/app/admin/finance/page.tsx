...
export default async function AdminFinancePage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminSectionHeader title="Finance plateforme" description="Pilotez le chiffre d'affaires CRESYVA, les coûts de revient, les charges, la marge et le bénéfice net." action={<Link href="/admin/payments" className="adm-btn-secondary">Paiements</Link>} />
      ...
      <AdminCard className="overflow-hidden border-primary/20 bg-navy-900 text-white">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-5">{[["2 gratuits", financial.zernio.freeAccounts], ["$6 / compte", financial.zernio.tier6Accounts], ["$3 / compte", financial.zernio.tier3Accounts], ["$1 / compte", financial.zernio.tier1Accounts], ["> 2 000 · 1 $", financial.zernio.over2000Accounts]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-white/5 p-4"><p className="text-[10px] uppercase tracking-wider text-white/40">{label}</p><p className="mt-1 text-2xl font-extrabold">{value}</p></div>)}</div>
        <p className="mt-4 text-[11px] leading-5 text-white/40">Estimation en rythme mensuel au nombre de comptes actuellement connectés. Zernio applique une proratisation quotidienne lorsque des comptes sont ajoutés ou retirés en cours de mois. Les frais d'usage variables de Zernio, numéros, X/Twitter, publicités ou autres services externes doivent être enregistrés séparément lorsqu'ils apparaissent sur la facture.</p>
      </AdminCard>
