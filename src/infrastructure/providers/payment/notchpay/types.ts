...
/**
 * NB (16/09/2026) : les types "Resources API" (listing pays/canaux
 * NotchPay) qui vivaient ici ont été retirés en même temps que
 * `resources-client.ts` et la synchronisation automatique associée —
 * cette dernière s'appuyait sur un endpoint qui n'existe pas
 * réellement côté NotchPay (404 constaté en prod). La gestion des
 * pays/canaux est désormais 100% manuelle (SQL + `/admin/countries`)
 * — voir docs/notchpay-resources.md pour l'historique complet et
 * docs/country-engine.md pour l'architecture actuelle.
 */
