# Rapport de fusion #14 — cresyva-fusionne (fusion #13) + Catalogue V2

Fait suite à `RAPPORT_FUSION_13.md`. Deux sources reçues :

1. **`cresyva-fusionne.zip`** (524 fichiers) — état livré par la fusion #13 :
   base CRESYVA + Google OAuth + lien « Console Admin » + Telegram
...
- Les fichiers de l'archive V2 portent tous la date de l'ancêtre, sauf
  **27** : ce sont exactement ceux du chantier Catalogue V2 (21 fichiers
  ...
  que V2 n'a jamais eues → version cresyva conservée telle quelle.
- Cresyva a été pris comme **base** (branche la plus avancée), puis les 27
  ...
  eux, les seules lignes propres à cresyva sont des lignes d'origine que V2
  a remplacées ou supprimées (cresyva = ancêtre sur ces fichiers) → version
...
`0056_promotion_deadline.sql`. Or `0055` est déjà pris côté cresyva par
...
Vérifié : aucune migration cresyva ne crée déjà `service_images`,
...
fichier que V2 n'a pas modifié : version cresyva conservée. Le
`tsconfig.tsbuildinfo` livré dans l'archive V2 (artefact de build) n'est pas
repris.

### 5. Vérifications de compatibilité inter-lots

- Aucun appelant, dans tout le code cresyva (y compris le lot Telegram v3),
...
- Diff final contre `cresyva-fusionne.zip` : uniquement les 21 fichiers V2
