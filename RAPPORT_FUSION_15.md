# Rapport de fusion #15 — trois livraisons sur cresyva-fusionne-14
...
- fichier touché d'un seul côté (cresyva = ancêtre) → version de la livraison ;
...
après la livraison de `cresyva-fusionne-14.zip`, donc absents de ce zip-là et
...
WhatsApp, le `package-lock.json` cresyva est conservé) :

| Traitement | Fichiers |
|---|---|
| Repris tels quels (cresyva = ancêtre) | `zernio/client.ts`, `zernio-channel-service.ts`, `providers/registry.ts`, `admin-addons-service.ts`, `admin/addons/page.tsx`, `admin/numbers/page.tsx` |
...
| Corrigé pour compatibilité entre lots (fichier cresyva, pas dans l'archive WhatsApp) | `omnichannel-publication-service.ts` |
...
- `dashboard/channels/page.tsx` : les deux côtés avaient modifié les mêmes deux
  lignes de la carte « WhatsApp Business ». Base **cresyva** (marque CRESYVA,
  ...
  texte d'origine devient « CRESYVA » sur cette surface d'interface.
...
### Vérifications d'interaction avec cresyva
...
- **Conflit sémantique réel, invisible dans un merge de texte, corrigé.**
  ...
  cresyva) envoyait immédiatement vers un groupe WhatsApp via
...
## Addendum — passage SME-OS → CRESYVA (version `15b`)
...
| Où | Avant | Après |
|---|---|---|
| Email d'invitation d'équipe (objet + corps) | « … sur SME-OS » | « … sur CRESYVA » |
| Description envoyée à NotchPay (vue par le payeur) | « Abonnement SME-OS — forfait … », « Add-on SME-OS : … », « SME-OS — Numéro WhatsApp dédié … » | idem avec CRESYVA |
| Notification d'échéance d'abonnement | « … continuer à utiliser SME-OS » | CRESYVA |
| Messages « pays indisponible / bientôt / liste d'attente » | « … sur SME-OS » | CRESYVA |
| `/dashboard/channels` (numéro dédié) | « l'équipe SME-OS », « que SME-OS m'en fournisse un » | CRESYVA |
| Titre YouTube par défaut (publication sans texte) | « Publication SME-OS » | « Publication CRESYVA » |
| Profil Zernio (nom de repli si l'entreprise n'a pas de nom, description) | « SME-OS … » | CRESYVA |
| Expéditeur d'email **par défaut** (`EMAIL_FROM_ADDRESS`) | `SME-OS <onboarding@resend.dev>` | `CRESYVA <onboarding@resend.dev>` |

Plus : commentaires de code, `globals.css`, `tailwind.config.ts`, `.env.example`,
`README.md` et 9 documents `docs/*.md` (docs vivantes).

### Décision — liens « Site propulsé par CRESYVA »

Le texte affiché disait déjà « CRESYVA » sur toutes les vitrines et landings
...
utilisée ailleurs pour les liens absolus). Raison : le domaine réel de CRESYVA
